...
def main():
    ...
    group_address = args.action + '/' + args.floor + '/' + args.bloc
    payload = [args.data, args.size, args.apci]

    endpoint_ip = '0.0.0.0'

    if args.mode == 'prod':
        gateway_ip = '129.194.184.31'
        gateway_port = 3671
        endpoint_port = 0
    elif args.mode == 'sim':
    ...
    print("Payload :          {:>20}".format(str(payload)[1:-1]))

    KNX_rw(data_endpoint, control_endpoint, gateway_ip, gateway_port, group_address, payload)
